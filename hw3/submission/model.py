...
class NaiveBayes:
    ...
    def fit(self, x, y):
        ...
        self.priors, self.gaussian, self.bernoulli, self.laplace, self.exponential, self.multinomial = self.get_params(x, y)
        """End of your code."""
    ...
    def predict(self, x):
        ...
        predictions = []
        for label in [0, 1, 2]:
            px1 = np.log(np.sqrt(2 * np.pi * self.gaussian[label][2]) * np.exp(-0.5 * ((x.T[0] - self.gaussian[label][0]) / self.gaussian[label][2]) ** 2))
            px2 = np.log(np.sqrt(2 * np.pi * self.gaussian[label][3]) * np.exp(-0.5 * ((x.T[1] - self.gaussian[label][1]) / self.gaussian[label][3]) ** 2))
    ...
    def get_params(x, y):
        ...
        for label in np.unique(y).astype(int):
            priors[label] = np.bincount(y.astype(int))[label] / len(x)
            ...
            multinomial[label] = [np.bincount(x_label.T[8].astype(int)) / len(x_label),
